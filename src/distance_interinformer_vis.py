...
def vectorizer_data (df):

    df["text_cleaned"] = df.text.apply(limpiar_texto)
    vectorizer_model = CountVectorizer(stop_words = create_stopwords_span())
    ...
    """
    The distance between informers can be calculated without the topics -1. 
    When we use BERTopic, some topics are assigned to -1 value. That means that these 
    topics are outliers and were not assigned to any important topic.
    ...
    df_vect = pd.DataFrame(
        doc_vect.toarray(), columns=vectorizer_model.get_feature_names_out()
    )
    ...
    Then, I calculated the mean of each component and use plotly. This IS NOT the best option.


    """

    X = df_to_distance.drop(
        columns=[
            "informer",
            "text",
            "text_cleaned",
            "text_cleaned_nstopwords",
            "num_topico",
        ]
    )
...
def main ():
    ...
    df = pd.read_csv('')
    df_to_distance = vectorizer_data (df)
    df_comp_means = (
        df_to_distance.groupby("informer")
        .agg({"Comp 1": "mean", "Comp 2": "mean"})
        .reset_index()
    )
    df_comp_means["size_point"] = 30  # I also added the size of the point in the chart.

    """
    Sometimes, it is necessary to standarize the data before PCA. In this case, it was not necessary, 
    but it depends on your data. I recommend to make df.describe() and df.info() 
    to be sure about what to do first.
    ...
    fig.add_shape(
        type="line",
        x0=mean_comp1_total,
        y0=mean_comp2_total - 2,
        x1=mean_comp1_total,
        y1=man_comp2_total + 2,
        line=dict(color="black", width=1),
    )

    fig.add_shape(
        type="line",
        x0=mean_comp1_total - 2,
        y0=mean_comp2_total,
        x1=mean_comp1_total + 2,
        y1=mean_comp2_total,
        line=dict(color="black", width=1),
    )
